      ...
      var s = [
        ...
        'histogram as (',
          'select array_agg(row(bucket, range, freq)) as hist from (',
               'select bucket, ',
                      'numrange(min + (bucket - 1) * (max - min),min + bucket * (max - min)) as range, ',
                      'count(1) as freq',
               'from (SELECT width_bucket({{column}}, min, max, 10) as bucket FROM ({{sql}}) _w1, stats) _w2, stats',
               'group by bucket, min, max',
               'order by bucket',
            ') __wrap',
          '),',
        'hist as (', 
