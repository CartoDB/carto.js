...
&#124;_ refreshTime | if is set, the layer is refreshed each refreshTime milliseconds. See a refreshTime code [example](https://github.com/CartoDB/cartodb.js/blob/develop/examples/createLayer_refresh_time.html). <br/><br/>**Tip:** To auto refresh and display the latest data in seconds, include the auto refresh time after the defined milliseconds in the code (i.e., `refreshTime: 2000 // 2 seconds`).
